var AjaxRestService = (function (logger, common, _, ServiceConfig, jQuery) {
    ...
    */
	AjaxService.prototype.loadConfig = function (config) {
		...
		this.successUpdateCallback = config.updateCallback || this.updateCallback;
	...
	AjaxService.prototype.internalUpdateSuccessCallback = function (data) {
		if (common.isJSON(data)){
			var codigoResultadoWS = JSON.parse(data).estado.split(":")[0].trim().toUpperCase();
		}
		else {
		...
		if (codigoResultadoWS === "OK") {
			if (!common.isEmpty(this.successUpdateCallback)) {
				this.successUpdateCallback(data);
			}
		} else {
			var jQuery_XHR = arguments[2];
			this.processServiceError(jQuery_XHR, codigoResultadoWS);
	...
	AjaxService.prototype.update = function (data, otraUrl) {
		var requestedUrl = '';

		if (!common.isEmpty(otraUrl)) {
			requestedUrl = common.combineUrl(otraUrl);
		} else {
			requestedUrl = common.combineUrl(this.baseUrl, this.postUrl);
		}
		if (this.serviceProvider === 'jquery') {
			...
			jQuery.ajax({
				...
				data: JSON.stringify(data),
				beforeSend: _.bind(this.beforeSendCallback, this),
				success: _.bind(this.internalUpdateSuccessCallback, this),
				error: _.bind(this.internalErrorCallback, this),
				complete: function (jqXHR, textStatus) { af.ui.hideMask(); }
			});
		} else if (serviceProvider === 'custom') {
